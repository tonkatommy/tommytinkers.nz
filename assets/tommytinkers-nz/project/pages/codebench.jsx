/* tommytinkers.nz · The Codebench — dev/digital landing with interactive build sheet */
const { useState: useCBState, useEffect: useCBEffect, useMemo: useCBMemo } = React;

/* =========================================================================
   DATA — project types, features per type, build recommendations
   ========================================================================= */

const PROJECT_TYPES = [
  { id: "marketing", icon: "globe", name: "Marketing site", blurb: "A few pages, a story to tell, a sign-up form." },
  { id: "webapp",    icon: "layout-dashboard", name: "Web app", blurb: "Logins, dashboards, real user data." },
  { id: "shop",      icon: "shopping-bag", name: "Online shop", blurb: "Products, checkout, NZ Post pickup." },
  ...
  { id: "unsure",    icon: "help-circle", name: "Not sure yet",       blurb: "Got an idea but it's still fuzzy. Talk it through." },
];
...
const TIMELINES = [
  ...
  { id: "month", icon: "calendar-clock", name: "Within a month or two", blurb: "Soonish, no fire." },
...
/* Recommendation engine — returns a tailored "build sheet" for the answers. */
function buildRecommendation({ type, features, timeline, budget }) {
  if (!type) return null;

  const BASE = {
    marketing: {
      headline: "Marketing site build",
      summary: "A fast, well-designed marketing site you can keep editing yourself.",
      stack: ["Next.js", "Sanity CMS", "Tailwind", "Vercel"],
      altStack: ["Astro", "MDX", "Cloudflare Pages"],
      priceLow: 1500, priceHigh: 4500,
      baseWeeks: 3,
      phases: [
        { key: "sketch",  name: "Sketch",  body: "Quick brief, sitemap, content audit. You sign off the shape." },
        { key: "design",  name: "Design",  body: "Hi-fi mocks of every unique page. Two rounds of revisions." },
        { key: "build",   name: "Build",   body: "Hand-coded, accessible, fast. CMS wired up so you can edit text yourself." },
        { key: "ship",    name: "Ship",    body: "DNS, analytics, a 30-minute handover call. You get the keys." },
      ],
      deliverables: [
        "Fully responsive site",
        "Editable in a friendly CMS",
        "Tracked analytics + SEO setup",
        "Documentation video for the CMS",
      ],
    },
    webapp: {
      headline: "Custom web app",
      summary: "A real product with logins, data, and a back office. Built to grow with you.",
      stack: ["Next.js", "Postgres", "TypeScript", "Stripe", "Auth.js"],
      altStack: ["Remix", "SQLite", "Drizzle"],
      priceLow: 5000, priceHigh: 18000,
      baseWeeks: 8,
      phases: [
        { key: "discover", name: "Discover", body: "Map the must-haves, the nice-to-haves, and what we drop. Get to an honest MVP." },
        { key: "design",   name: "Design",   body: "Flows + screens for the core journeys. Test with one real user before code." },
        { key: "mvp",      name: "MVP build",body: "Smallest useful slice, shipped fast. You can use it end-to-end in a few weeks." },
        { key: "iterate",  name: "Iterate",  body: "Add the next feature only once the previous one is paying for itself." },
      ],
      deliverables: [
        "Working app on your domain",
        "Admin panel for you to manage data",
        "Code in your GitHub, hosted on your account",
        "Runbook so you (or your next dev) can keep it alive",
      ],
    },
    shop: {
      headline: "Online shop build",
      summary: "Sell things online without renting a giant platform. Plays nice with NZ Post.",
      stack: ["Next.js", "Stripe", "Sanity", "NZ Post API"],
      altStack: ["Shopify Hydrogen", "Snipcart"],
      priceLow: 2500, priceHigh: 9000,
      baseWeeks: 5,
      phases: [
        { key: "catalogue", name: "Catalogue", body: "Get your products, prices, and photos shaped up. Shipping rules sorted." },
        { key: "design",    name: "Design",    body: "Storefront mocks. Cart, checkout, and the small things buyers notice." },
        { key: "build",     name: "Build",     body: "Storefront, payments, NZ Post tracking, order emails. End-to-end test." },
        { key: "ship",      name: "Ship",      body: "Soft-launch with a friends-only code, then open the doors." },
      ],
      deliverables: [
        "Storefront that loads fast on mobile",
        "Stripe (or POLi NZ) checkout",
        "Order admin you can run from your phone",
        "NZ-shipping rules and tracking integration",
      ],
    },
    tool: {
      headline: "Internal tool / dashboard",
      summary: "Replace the spreadsheet, the inbox-as-database, the worst part of someone's week.",
      stack: ["Next.js", "Postgres", "Tailwind"],
      altStack: ["Retool", "Airtable + scripts"],
      priceLow: 1500, priceHigh: 8000,
      baseWeeks: 4,
      phases: [
        { key: "audit",  name: "Audit",  body: "Shadow the people doing the work. Find what actually slows them down." },
        { key: "design", name: "Design", body: "Sketch the screens with them in the room. Cut anything they don't use." },
        { key: "build",  name: "Build",  body: "Ship the smallest tool that replaces the worst step. Add from there." },
        { key: "train",  name: "Train",  body: "One short training session, one printed cheat-sheet. Then leave them to it." },
      ],
      deliverables: [
        "The tool itself, on your domain or intranet",
        "Login that fits how your team already works",
        "Source code + setup notes handed over",
        "Two weeks of free tweaks after go-live",
      ],
    },
    automate: {
      headline: "Automation / script",
      summary: "A small program that does the boring thing every day so you don't.",
      stack: ["Node.js", "TypeScript", "GitHub Actions"],
      altStack: ["Python", "AWS Lambda"],
      priceLow: 400, priceHigh: 3500,
      baseWeeks: 1,
      phases: [
        { key: "trace",  name: "Trace",  body: "Watch you do the task once. Note every click and decision." },
        { key: "build",  name: "Build",  body: "Write the script. Test against last month's real data." },
        { key: "deploy", name: "Deploy", body: "Schedule it, set alerts so you know if it ever breaks." },
        { key: "hand",   name: "Hand-over", body: "You get the source, the schedule, and a one-page runbook." },
      ],
      deliverables: [
        "The script, in your repo",
        "Runs on a schedule (or trigger) you control",
        "Failure alerts straight to your email or Slack",
        "Plain-English runbook",
      ],
    },
    unsure: {
      headline: "Let's figure it out",
      summary: "A 30-minute call where we work out the shape of the thing. No pressure, no quote yet.",
      stack: ["A whiteboard", "Honest questions", "Coffee"],
      altStack: [],
      priceLow: 0, priceHigh: 0,
      baseWeeks: 0,
      phases: [
        { key: "talk",   name: "Talk",   body: "Tell me the problem, the people, the deadline. I'll ask the awkward questions." },
        { key: "shape",  name: "Shape",  body: "Sketch one or two ways it could go. Maybe it's a build, maybe it's a spreadsheet." },
        { key: "decide", name: "Decide", body: "You get a written summary + ballpark. Use it with me, or take it to someone else." },
      ],
      deliverables: [
        "30-minute discovery call (free)",
        "Written one-pager summarising the idea",
        "Ballpark cost + timeline",
        "Honest recommendation — even if it's 'don't build it yet'",
      ],
    },
  };

  const rec = JSON.parse(JSON.stringify(BASE[type]));

  // Bump price / weeks based on feature count
  ...
  // Timeline bump — rush adds a small surcharge note
...
const fmt$ = (n) => "NZ$" + n.toLocaleString("en-NZ");
      ...
      <div className="cb-hero-grid">
        <div>
          ...
          <div className="cb-hero-meta">
            <span><i data-lucide="check-circle-2" style={{ width: 14, height: 14 }}></i> Available from June</span>
            <span><i data-lucide="map-pin" style={{ width: 14, height: 14 }}></i> Helensville, NZ</span>
            <span><i data-lucide="clock" style={{ width: 14, height: 14 }}></i> Replies within 1 working day</span>
          </div>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 24 }}>
            <a className="tt-btn tt-btn-warm" href="#build-sheet">
              <i data-lucide="hammer" style={{ width: 14, height: 14 }}></i> Sketch a project
            </a>
            <a className="tt-btn tt-btn-ghost" href="#what-i-build">
              <i data-lucide="arrow-down" style={{ width: 14, height: 14 }}></i> See what I build
            </a>
          </div>
        </div>
...
const SERVICES = [
  { icon: "globe-2", title: "Websites", body: "Marketing sites, portfolios, microsites. Fast, accessible, easy for you to edit.", eg: "kaipara-co.nz · 1 week" },
  { icon: "layout-dashboard", title: "Web apps", body: "Logins, dashboards, real user data. Built so a future dev can read the code.", eg: "Bookkit · 6 weeks" },
    ...
    <div className="tt-container">
      <div className="tt-section-header">
        ...
        <p className="tt-section-sub">
          Roughly. Most projects are a mix. If it lives on the internet and someone needs it built, the answer is usually yes.
        </p>
      </div>
        ...
        {SERVICES.map((s) => (
          <article key={s.title} className="tt-card cb-service-card">
            <div className="cb-service-icon">
              <i data-lucide={s.icon} style={{ width: 22, height: 22, strokeWidth: 1.8 }}></i>
            </div>
            <h4>{s.title}</h4>
            <p>{s.body}</p>
...
const ChoiceCard = ({ choice, selected, onClick }) => (
  <button type="button" className="cb-choice" aria-pressed={selected} onClick={onClick}>
    <div className="cb-choice-icon">
      <i data-lucide={choice.icon} style={{ width: 18, height: 18, strokeWidth: 2 }}></i>
    </div>
    <div className="cb-choice-text">
      <strong>{choice.name}</strong>
      <span>{choice.blurb}</span>
    </div>
    <span className="cb-choice-check">
      {selected && <i data-lucide="check" style={{ width: 11, height: 11, strokeWidth: 3 }}></i>}
    </span>
  </button>
);
...
/* ---------- Build sheet (right column) ---------- */
const BuildSheet = ({ state, rec }) => {
        ...
        {rec && (
          <>
            ...
            <div className="cb-sheet-row">
              ...
              <span className="cb-sheet-val">
                {rec.stack.map((s) => <span key={s} className="stack-tag">{s}</span>)}
              </span>
            </div>
            <div className="cb-sheet-row">
              <span className="cb-sheet-key">estimate</span>
              <span className="cb-sheet-val ok">
                {rec.priceLow === 0 ? "Free 30-min call" : `${fmt$(rec.priceLow)} – ${fmt$(rec.priceHigh)}`}
              </span>
            </div>
            <div className="cb-sheet-row">
              <span className="cb-sheet-key">duration</span>
              <span className="cb-sheet-val">
                {rec.baseWeeks === 0 ? "30 minutes" : `~${rec.baseWeeks} week${rec.baseWeeks === 1 ? "" : "s"}`}
              </span>
            </div>
          </>
        )}
...
const Step1 = ({ state, set }) => (
  <div>
    ...
    <div className="cb-step-nav">
      <a href="index.html" className="tt-btn tt-btn-ghost" style={{ fontFamily: "var(--font-mono)", fontSize: 13 }}>
        <i data-lucide="arrow-left" style={{ width: 14, height: 14 }}></i> Cancel
      </a>
      <button className="tt-btn tt-btn-primary" disabled={!state.type} onClick={() => set({ step: 2 })}
        ...
        Next <i data-lucide="arrow-right" style={{ width: 14, height: 14 }}></i>
      </button>
    </div>
  </div>
);
  ...
  return (
    <div>
      ...
      <div className="cb-chips">
        {opts.map((f) => (
          <Chip key={f} active={state.features.includes(f)} onClick={() => toggle(f)} icon={state.features.includes(f) ? "check" : "plus"}>
            {f}
          </Chip>
        ))}
      </div>
      <div className="cb-step-nav">
        <button className="tt-btn tt-btn-ghost" onClick={() => set({ step: 1 })}>
          <i data-lucide="arrow-left" style={{ width: 14, height: 14 }}></i> Back
        </button>
        <button className="tt-btn tt-btn-primary" onClick={() => set({ step: 3 })}>
          Next <i data-lucide="arrow-right" style={{ width: 14, height: 14 }}></i>
        </button>
      </div>
    </div>
...
const Step3 = ({ state, set }) => (
  <div>
    ...
    <div className="cb-choices" style={{ marginBottom: 24 }}>
      {TIMELINES.map((t) => (
        <ChoiceCard key={t.id} choice={t} selected={state.timeline === t.id} onClick={() => set({ timeline: t.id })} />
      ))}
    </div>
    ...
    <div className="cb-chips">
      {BUDGETS.map((b) => (
        <Chip key={b.id} active={state.budget === b.id} onClick={() => set({ budget: b.id })}>
          {b.name}
        </Chip>
      ))}
    </div>
    ...
    <div className="cb-step-nav">
      <button className="tt-btn tt-btn-ghost" onClick={() => set({ step: 2 })}>
        <i data-lucide="arrow-left" style={{ width: 14, height: 14 }}></i> Back
      </button>
      <button className="tt-btn tt-btn-primary" disabled={!state.timeline || !state.budget}
        onClick={() => set({ step: 4 })}
        style={{ opacity: (state.timeline && state.budget) ? 1 : 0.45, cursor: (state.timeline && state.budget) ? "pointer" : "not-allowed" }}>
        See my build sheet <i data-lucide="sparkles" style={{ width: 14, height: 14 }}></i>
      </button>
    </div>
  </div>
);
...
/* ---------- Step 4: Result ---------- */
const Step4 = ({ state, set, rec }) => {
  const [sent, setSent] = useCBState(false);
  const [extra, setExtra] = useCBState({ name: "", email: "", note: "" });
  ...
  const mailto = `mailto:tommy@tommytinkers.nz?subject=${encodeURIComponent("Codebench brief — " + (rec ? rec.headline : ""))}&body=${encodeURIComponent(briefSummary)}`;
  ...
  return (
    <div>
      ...
      <div className="cb-result">
        ...
        {rec.rushNote && (
          <div style={{
            fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--color-warm-orange-deep)",
            background: "var(--color-accent-subtle)", border: "1px dashed var(--color-accent-muted)",
            padding: "10px 14px", borderRadius: 8,
          }}>
            <i data-lucide="info" style={{ width: 13, height: 13, verticalAlign: "-2px", marginRight: 6 }}></i>
            {rec.rushNote}
          </div>
        )}
        ...
        <div className="cb-result-cols">
          <div className="cb-result-block">
            ...
            <ul>
              {rec.stack.map((s) => (
                <li key={s}><i data-lucide="check" style={{ width: 14, height: 14, strokeWidth: 3 }}></i>{s}</li>
              ))}
            </ul>
            {rec.altStack && rec.altStack.length > 0 && (
          ...
          <div className="cb-result-block">
            ...
            <ul>
              {rec.deliverables.map((d) => (
                <li key={d}><i data-lucide="package" style={{ width: 14, height: 14 }}></i>{d}</li>
              ))}
            </ul>
          </div>
        </div>
          ...
          <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div className="cb-handoff-row">
              <button type="submit" className="tt-btn tt-btn-primary">
                <i data-lucide="send" style={{ width: 14, height: 14 }}></i> Send the brief
              </button>
              <a className="tt-btn tt-btn-ghost" href={mailto}>
                <i data-lucide="mail" style={{ width: 14, height: 14 }}></i> Or open in email
              </a>
              <button type="button" className="tt-btn tt-btn-ghost"
                onClick={() => set({ step: 1, type: "", features: [], timeline: "", budget: "" })}
                style={{ marginLeft: "auto" }}>
                <i data-lucide="rotate-ccw" style={{ width: 14, height: 14 }}></i> Start over
              </button>
            </div>
            {sent && (
              <div style={{
                fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--color-success)",
                background: "rgba(76,175,46,0.08)", border: "1px solid rgba(76,175,46,0.3)",
                padding: "10px 14px", borderRadius: 8,
              }}>
...
const Wizard = () => {
  const [state, setStateRaw] = useCBState(() => {
    try {
  ...
  const set = (patch) => setStateRaw((s) => {
    const next = { ...s, ...patch };
    try { localStorage.setItem("tt-codebench-v1", JSON.stringify(next)); } catch (e) {}
    return next;
  });

  useCBEffect(() => { if (window.lucide) window.lucide.createIcons(); }, [state]);

  const rec = useCBMemo(() => buildRecommendation(state), [state.type, state.features, state.timeline, state.budget]);
    ...
    <section id="build-sheet" className="cb-wiz-section">
      <div className="tt-container">
        <div className="tt-section-header">
          ...
          <p className="tt-section-sub">
            Four quick questions. As you answer, the build sheet on the right fills itself in — stack, phases, ballpark, and what you'd actually get. No sign-up. Saves to this device.
          </p>
        </div>
      ...
      <div className="cb-work-grid">
        {PAST_WORK.map((w) => (
          <article key={w.name} className="tt-card cb-work-card">
            ...
            <h4>{w.name}</h4>
            <p>{w.body}</p>
            <div className="cb-work-tags">
              {w.tags.map((t) => <span key={t} className="tt-tag">{t}</span>)}
            </div>
          </article>
        ))}
      </div>
...
const CodebenchPage = () => {
  useCBEffect(() => { if (window.lucide) window.lucide.createIcons(); });
  return (
    <>
      <TTNav active="codebench" />
      <main>
        <CodebenchHero />
        <CodebenchServices />
        <Wizard />
...
ReactDOM.createRoot(document.getElementById("root")).render(<CodebenchPage />);
